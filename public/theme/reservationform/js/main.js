jQuery( document ).ready(function( $ ) {
	...
	$(form).submit(function(){
		...
		$.ajax({
			type: 'POST',
			url: 'assets/reservationform/script.php',
			data: $(form).serialize(),
			dataType: 'json',
			success: function(data){
				...
				}else if (data.code == 'failed-recaptcha'){
					$('#booking-form').append($('<div>').addClass('error-message').html('Robot verification failed, please try again.'));
